...
public class FoodBolusInverse extends RenderableComponentBase 
   implements ForceComponent, ScalableUnits, TransformableGeometry, HasNumericState
{

   boolean useNormalToPlane = false; // if false, calculate normal b/w plane origin and colliding point
   
   protected double distanceThreshold = 0.5;
   protected boolean debug = false;

   PointPlaneForce myPlane = null;
   ...
   public static PropertyList myProps = new PropertyList(FoodBolusInverse.class,
   ...
   public FoodBolusInverse()
    {
      super();
      myRenderProps = createRenderProps ();
    }

   public FoodBolusInverse (String name, PointPlaneForce plane, double diam, double resist, double damp)
    { 
      this();
      setName (name);
      myPlane = plane;
   ...
   public PointPlaneForce getPlane()
    {
      return myPlane;
    }
   
   public void setPlane(PointPlaneForce p)
    {
      myPlane = p;
    }
   ...
   public double getDistanceToPlane() {
      double offset = myPlane.getOffset();
...
//      double offset = myNormal.dot(XPB.p);
   ...
   public double getDistanceToPlaneOrigin() {
      //RigidTransform3d TDW = myPlane.getCurrentTDW();
      myNormal.sub(collidingPt.getPosition (), myPlane.getCenter ());
   ...
   public void applyForces (double t) {
      ...
      if (useNormalToPlane) {
         dist = getDistanceToPlane ();
      }
      else {
         dist = getDistanceToPlaneOrigin ();
      }
