...
public class JawModelFEMSensivity extends JawModel{
   ...
   protected double RGHDistance=0;
   protected double RAMDistance=0;
   protected double RADDistance=0;
   protected double RSPDistance=0;

   protected double LDMDistance=0;
   protected double LATDistance=0;
   protected double LMPDistance=0;
   protected double LSPDistance=0;
  ...
  public ArrayList<Muscle> myAttachedMuscles = new ArrayList<Muscle>();
   ...
   new PropertyList (JawModelFEMSensivity.class, JawModel.class);
   ...
   static {
      myProps.add("LDM", "LDM Distance", 0);
      myProps.add("LAT", "LAT Distance", 0);
      myProps.add("LMP", "LMP Distance", 0);
      myProps.add("LSP", "LSP Distance", 0);
      myProps.add("RGH", "RGH Distance", 0);
      myProps.add("RAM", "RAM Distance", 0);
      myProps.add("RAD", "RAD Distance", 0);
      myProps.add("RSP", "RSP Distance", 0);
   ...
   public double getLDM() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("ldm_insertion").getLocation ().x - frameMarkers ().get ("LDMDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("ldm_insertion").getLocation ().y - frameMarkers ().get ("LDMDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("ldm_insertion").getLocation ().z - frameMarkers ().get ("LDMDup").getLocation ().z,2);
      
      return (double) Math.round(Math.sqrt (term1 + term2 + term3)*100)/100;
   }
   
   
   public void setLDM(double LDM) {
      LDMDistance = frameMarkers ().get ("LDM_insertion").getLocation ().x;
   ...
   public double getLAT() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("lat_insertion").getLocation ().x - frameMarkers ().get ("LATDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("lat_insertion").getLocation ().y - frameMarkers ().get ("LATDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("lat_insertion").getLocation ().z - frameMarkers ().get ("LATDup").getLocation ().z,2);
   ...
   public void setLAT(double LAT) {
      LATDistance = frameMarkers ().get ("LAT_insertion").getLocation ().x;
   ...
   public double getLMP() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("lmp_insertion").getLocation ().x - frameMarkers ().get ("LMPDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("lmp_insertion").getLocation ().y - frameMarkers ().get ("LMPDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("lmp_insertion").getLocation ().z - frameMarkers ().get ("LMPDup").getLocation ().z,2);
   ...
   public void setLMP(double LMP) {
      LMPDistance = frameMarkers ().get ("LMP_insertion").getLocation ().x;
   ...
   public double getLSP() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("lsp_insertion").getLocation ().x - frameMarkers ().get ("LSPDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("lsp_insertion").getLocation ().y - frameMarkers ().get ("LSPDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("lsp_insertion").getLocation ().z - frameMarkers ().get ("LSPDup").getLocation ().z,2);
      
      return (double) Math.round(Math.sqrt (term1 + term2 + term3)*100)/100;
   }
   
   
   public void setLSP(double LSP) {
      LSPDistance = frameMarkers ().get ("LSP_insertion").getLocation ().x;
   ...
   public double getRGH() {
       
      double term1 = Math.pow(frameMarkers ().get ("rgh_insertion").getLocation ().x - frameMarkers ().get ("RGHDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("rgh_insertion").getLocation ().y - frameMarkers ().get ("RGHDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("rgh_insertion").getLocation ().z - frameMarkers ().get ("RGHDup").getLocation ().z,2);
      
      return (double) Math.round(Math.sqrt (term1 + term2 + term3)*100)/100;
   }
   
   
   public void setRGH(double RGH) {
      RGHDistance = frameMarkers ().get ("rgh_insertion").getLocation ().x;
   ...
   public double getRAM() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("ram_origin").getLocation ().x - frameMarkers ().get ("RAMDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("ram_origin").getLocation ().y - frameMarkers ().get ("RAMDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("ram_origin").getLocation ().z - frameMarkers ().get ("RAMDup").getLocation ().z,2);
   ...
   public void setRAM(double RAM) {
      RAMDistance = frameMarkers ().get ("ram_origin").getLocation ().x;
 ...
 public double getRAD() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("rad_insertion").getLocation ().x - frameMarkers ().get ("RADDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("rad_insertion").getLocation ().y - frameMarkers ().get ("RADDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("rad_insertion").getLocation ().z - frameMarkers ().get ("RADDup").getLocation ().z,2);
   ...
   public void setRAD(double RAD) {
      RADDistance = frameMarkers ().get ("rad_insertion").getLocation ().x;
 ...
 public double getRSP() {
      ...
      double term1 = Math.pow(frameMarkers ().get ("rsp_insertion").getLocation ().x - frameMarkers ().get ("RSPDup").getLocation ().x,2);
      double term2 = Math.pow(frameMarkers ().get ("rsp_insertion").getLocation ().y - frameMarkers ().get ("RSPDup").getLocation ().y,2);
      double term3 = Math.pow(frameMarkers ().get ("rsp_insertion").getLocation ().z - frameMarkers ().get ("RSPDup").getLocation ().z,2);
   ...
   public void setRSP(double RSP) {
      RSPDistance = frameMarkers ().get ("rsp_insertion").getLocation ().x;
   ...
   */
   ...
   public PolygonalMesh loadFemMesh(String meshName, double scale){
      String meshFilename = ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,
         "geometry/" + meshName);
...
// Creates and returns a ColorBar renderable object
   ...
   public void addCapsule() throws IOException {      
      ...
      FemModel3d caps_right = new FemModel3d ("capsule_r");
       ...
       ArrayList<Integer> attached_points_l_man = readIntList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry"+"/capsule_l_fixedNodes_man_wedge_fine_5.txt"));
       ...
       ArrayList<Integer> attached_points_l_skull = readIntList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry"+"/capsule_l_fixedNodes_skull_wedge_fine_5.txt"));
       ...
       ArrayList<Integer> attached_points_r_man = readIntList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry"+"/capsule_r_fixedNodes_man_wedge_fine_19.txt"));   
       ...
       ArrayList<Integer> attached_points_r_skull = readIntList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry"+"/capsule_r_fixedNodes_skull_wedge_fine_19.txt"));
   ...
   public JawModelFEMSensivity () throws IOException {
      super();
   }

   
   public JawModelFEMSensivity (String name) throws IOException {
      super();
      this.setName (name);
      ...
      JawModel.muscleList = readStringList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+muscleListFilename));
      JawModel.bodyInfoList = readBodyInfoList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+bodyListFilename));
      femInfoList = readFemInfoList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+femListFilename));
      
      JawModel.muscleInfo = readMuscleInfo(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+muscleInfoFilename));
      JawModel.muscleGroupInfo = readMuscleGroupsInfo(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+muscleGroupInfoFilename));
      ArrayList<RigidBody> bodies = JawModel.assembleRigidBodies(bodyInfoList, amiraTranformation,ArtisynthPath.getSrcRelativePath (JawModelFEMSensivity.class, ""));
      ...
      String meshFilename = (ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry"+"/mandible_with_cartilage2.obj"));
      PolygonalMesh mesh = new PolygonalMesh();
      try {
         mesh.read(new BufferedReader(new FileReader(meshFilename)));
      } catch (IOException e) {
      ...
      ArrayList<FrameMarker>markers = JawModel.assembleMarkers(muscleList, muscleInfo, myRigidBodies, amiraTranformation, ArtisynthPath.getSrcRelativePath (JawModelFEMSensivity.class, ""));
      ...
      wrappedMuscleList = readStringList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+wrappedMuscleListFilename));
      ...
      */
      ...
      closerMuscleList = JawModel.createMuscleList(readStringList(ArtisynthPath.getSrcRelativePath(JawModelFEMSensivity.class,"geometry/"+"closerMuscleList.txt")), muscleInfo, myAttachedMuscles);
   ...
   public void addDuplicateFrameMarker() {
      FrameMarker RGHDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (-5.64512, -30.954, -32.2108));
      RGHDup.setName ("RGHDup");
      addFrameMarker (RGHDup);
      RenderProps.setPointColor (RGHDup, Color.RED);
      
      FrameMarker RAMDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (-16.0677, -16.6112, -21.0049));
      RAMDup.setName ("RAMDup");
      addFrameMarker (RAMDup);
      RenderProps.setPointColor (RAMDup, Color.RED);
      
      FrameMarker RADDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (-14.0062, -25.8835, -31.678));
      RADDup.setName ("RADDup");
      addFrameMarker (RADDup);
      RenderProps.setPointColor (RADDup, Color.RED);
      
      FrameMarker RSPDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (-48.6763, 40.4074, 40.2093));
      RSPDup.setName ("RSPDup");
      addFrameMarker (RSPDup);
      RenderProps.setPointColor (RSPDup, Color.RED);
      
      
      
      
      FrameMarker LDMDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (48.9422, 26.3876, 16.8474));
      LDMDup.setName ("LDMDup");
      addFrameMarker (LDMDup);
      RenderProps.setPointColor (LDMDup, Color.RED);
      
      FrameMarker LATDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (37.923, 17.369, 22.035));
      LATDup.setName ("LATDup");
      addFrameMarker (LATDup);
      RenderProps.setPointColor (LATDup, Color.RED);
      
      FrameMarker LMPDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (41.8049, 23.3688, -9.75978));
      LMPDup.setName ("LMPDup");
      addFrameMarker (LMPDup);
      RenderProps.setPointColor (LMPDup, Color.RED);
      
      FrameMarker LSPDup = new FrameMarker(rigidBodies ().get ("jaw"), new Point3d (51.2801, 38.9711, 41.3814));
      LSPDup.setName ("LSPDup");
      addFrameMarker (LSPDup);
      RenderProps.setPointColor (LSPDup, Color.RED);
