   ...
   implements ForceComponent, ScalableUnits, TransformableGeometry, HasNumericState
{
   ...
   protected double distanceThreshold = 0.1;
...
//      double offset = myNormal.dot(XPB.p);
   ...
   public double getDistanceToPlaneOrigin(Vector3d normal) {
      ...
      double dist = Math.abs(myNormal.z);
      myNormal.normalize ();
      return dist;
   }
...
//  	   System.out.println("dist = " + dist +"; v = " + v + ";diam = " + myDiameter);
   ...
   public void printPointReferences(PrintWriter pw, CompositeComponent ancestor)
	 ...
	 * throws IOException {
	 * 
	 * rtok.nextToken(); if (scanAndStoreReference (rtok, "point", tokens)) { return
	 * true; } else if (scanAttributeName (rtok, "planarConnector")) { myPlane = new
	 * PlanarConnector(); tokens.offer (new StringToken ("planarConnector",
	 * rtok.lineno())); myPlane.scan (rtok, tokens); return true; } rtok.pushBack();
	 * return super.scanItem (rtok, tokens); }
	 */

	/*
	 * protected void writeItems ( PrintWriter pw, NumberFormat fmt,
	 * CompositeComponent ancestor) throws IOException {
	 * 
	 * pw.println ( "point=" + ComponentUtils.getWritePathName(ancestor,
	 * collidingPt)); pw.print ("planarConnector="); myPlane.write (pw, fmt,
	 * ancestor); super.writeItems (pw, fmt, ancestor); }
	 */
   ...
   // }
	...
	/*
	 * protected boolean postscanItem ( Deque<ScanToken> tokens, CompositeComponent
	 * ancestor) throws IOException {
	 * 
	 * if (postscanAttributeName (tokens, "point")) { collidingPt =
	 * postscanReference (tokens, Point.class, ancestor); return true; } else if
	 * (ScanWriteUtils.postscanAttributeName (tokens, "planarConnector")) {
	 * myPlane.postscan (tokens, ancestor); return true; } return super.postscanItem
	 * (tokens, ancestor); }
	 */
