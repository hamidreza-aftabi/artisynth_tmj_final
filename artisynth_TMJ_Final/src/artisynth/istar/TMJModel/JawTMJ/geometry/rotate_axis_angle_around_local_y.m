function [new_axis, new_angle] = rotate_axis_angle_around_local_y(axis, angle_deg, alpha_deg)
    % Normalize the axis
    axis = axis / norm(axis);
    ...
    % Additional rotation around the local y-axis by alpha degrees
    alpha_rad = deg2rad(alpha_deg);
    R_y_local = [cos(alpha_rad), 0, sin(alpha_rad);
                 0, 1, 0;
                 -sin(alpha_rad), 0, cos(alpha_rad)];
    
    % Combine the rotations
    R_combined = R * R_y_local;
