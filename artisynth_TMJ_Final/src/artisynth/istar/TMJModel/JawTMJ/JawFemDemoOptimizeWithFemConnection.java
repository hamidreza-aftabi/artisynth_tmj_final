...
public class JawFemDemoOptimizeWithFemConnection extends RootModel implements ActionListener {
   ...
   String myGeoDir = PathFinder.getSourceRelativePath (
      JawFemDemoOptimizeWithFemConnection.class, "geometry/");
   ...
   public static PropertyList myProps = new PropertyList (JawFemDemoOptimizeWithFemConnection.class, RootModel.class);
...
/*
   ...
   */
   ...
   public JawFemDemoOptimizeWithFemConnection () {
      
   }

   
   public JawFemDemoOptimizeWithFemConnection (String name){
      super(null);
   }
   ...
   public void build (String[] args) throws IOException {
      ...
      connection = createFemModel (
         myJawModel, "connection", "connection.obj");
      
      LinearMaterial corticalBoneMaterial0 = new LinearMaterial(corticalBoneYoungModulus, corticalBonePoissonRatio);

      connection.setDensity (corticalBoneDensity);
      connection.setMaterial (corticalBoneMaterial0);
      ...
      connection.setComputeNodalStress (true);
      connection.setComputeNodalStrain (true);
      connection.setComputeStrainEnergy (true);
      connection.setComputeNodalEnergyDensity (true);
     ...
     PolygonalMesh mandibleMesh = myJawModel.rigidBodies().get ("jaw").getSurfaceMesh();
    
     int nattach = 0;
     for (FemNode3d n : connection.getNodes()) {
        if (mandibleMesh.distanceToPoint (n.getPosition()) <= 0.05) {
           myJawModel.attachPoint (n, myJawModel.rigidBodies().get ("jaw"));
           nattach++;
        }
     }
     ...
     connectionMesh = (RigidBody)myJawModel.findComponent (
     "rigidBodies/connection_mesh");
     connectionMesh.setDynamic (false);
     
     surfaceLeft = connectionMesh.getSurfaceMesh();
  ...
  elementsCloseToSurface = new HashSet<>();
   ...
   private void setDonorSegmentInteractions (MechModel mech) {
      ...
      myJawModel.setCollisionBehavior ((FemModel3d) myJawModel.models().get ("connection"), (FemModel3d) myJawModel.models().get ("donor0"), behav8);
   ...
   public void setWorkingDir() {
      ...
      ArtisynthPath.getSrcRelativePath(JawFemDemoOptimizeWithFemConnection.class, workingDirname));
